...
save_path = "traindModel/model-8400"

INITIAL_LN_RATE = 0.0001  # 学习率
DECAY_STEPS = 50000
DECAY_FACTOR = 0.99  # The learning rate decay factor

inputs = tf.placeholder(tf.float32, [None, image_height, image_width, input_channel])  # 指定输入张量
...
output = tf.reshape(output, [-1, lst[2] * lst[3]])  # [batch*width, height*channels]  注意这里不能是lst[0]*lst[1]，因为他俩不确定，不能计算它们
...
u_out = tf.reshape(u_out, [-1, lst[1], class_num])  # [batch, max_time, num_classes]，注意这里不能是lst[0]和lst[1]，因为他俩不确定，不能计算它们
u_out = tf.transpose(u_out, (1, 0, 2))  # [max_time, batch, num_classes]

loss = tf.nn.ctc_loss(labels=targets, inputs=u_out, sequence_length=seq_len * lst[1])
...
# 计算精确度
decoded, log_prob = tf.nn.ctc_beam_search_decoder(u_out, seq_len * lst[1], merge_repeated=False)
...
with tf.Session() as sess:
    ...
    while True:
        ...
        for j in range(10):
            opt, dec, dis = sess.run([optimizer, decoded, distance],
                                feed_dict={inputs: img, targets: sparse_targets, seq_len: seq_len1, is_training: True})
        print("第{}次：{}".format(i,dis))
        if i%100 == 0 and i!=0:
            img, sparse_targets, seq_len1 = textGen.get_next_batch(1)
            dec, dis= sess.run([decoded, distance], feed_dict={inputs: img, targets: sparse_targets, seq_len: seq_len1, is_training: False})
